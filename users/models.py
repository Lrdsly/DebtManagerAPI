...
class CustomUser(AbstractBaseUser, CPermissionMixin):
    ...
    room = models.ManyToManyField(
        verbose_name=_("Room"), to="users.Room", through="users.MemberShip"
    )
...
class Notification(models.Model):
    ...
    text = models.TextField(
        _("notification text"), max_length=500, blank=True, null=True
    )
...
class Room(models.Model):
    ...
    name = models.CharField(
        verbose_name=_("Room name"), max_length=100, blank=True, null=True
    )
    slug = models.SlugField(
        verbose_name=_("Room slug"), max_length=100, unique=True
    )  # add auto generate slug later
    admin = models.ForeignKey(
        verbose_name=_("Room admin"),
        to="users.CustomUser",
        on_delete=models.CASCADE,
        related_name="owned_rooms",
    )
    mode = models.IntegerField(
        verbose_name=_("Room status"), choices=ModeChoice, default=1
    )
...
class MemberShip(models.Model):
    ...
    user = models.ForeignKey(
        "users.CustomUser", on_delete=models.CASCADE, related_name="joined_rooms"
    )
    room = models.ForeignKey(
        "users.Room", on_delete=models.CASCADE, related_name="members"
    )
...
class FriendShip(models.Model):
    ...
    user1 = models.ForeignKey(
        "users.CustomUser", on_delete=models.CASCADE, related_name="friendship_user1"
    )
    user2 = models.ForeignKey(
        "users.CustomUser", on_delete=models.CASCADE, related_name="friendship_user2"
    )
