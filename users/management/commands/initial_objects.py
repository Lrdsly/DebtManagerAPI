...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        global_room, global_room_created = Room.objects.get_or_create(
            slug="global", defaults={"name": "Global", "admin": admin}
        )
        ...
        member_ship, member_ship_created = MemberShip.objects.get_or_create(
            user=admin, room=global_room
        )
        if member_ship_created:
            self.stdout.write(self.style.SUCCESS("Admin User Joined Global Room."))
        else:
            self.stdout.write(
                self.style.NOTICE("Admin user is already a global room member.")
            )
