...
from users.models import MemberShip, Notification, Room
...
class PublicUserSerializer(serializers.ModelSerializer):
    ...
    def get_rooms(self, obj):
        return MemberShip.objects.filter(user=obj).values_list(
            "room_id__name", flat=True
        )
...
class RoomSerializer(serializers.ModelSerializer):
    ...
    def get_members_property(self, obj):
        members = MemberShip.objects.filter(room=obj).values_list(
            "user__username", flat=True
        )
        members_property = []
        for username in members:
            total_debt = counters.get_user_total_debt(obj.slug, username)
            members_property.append({"username": username, "total_debt": total_debt})
